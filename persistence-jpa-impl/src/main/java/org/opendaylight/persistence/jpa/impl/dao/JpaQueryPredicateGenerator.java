 ...
 */
package org.opendaylight.persistence.jpa.impl.dao;
...
import org.opendaylight.persistence.util.common.converter.IdentityConverter;
...
public class JpaQueryPredicateGenerator<P> {
    ...
    /*
     * NOTE: There are several ways of implementing the singleton pattern, some of them more secure than others
     * guaranteeing that one and only one instance will exists in the system (taking care of deserialization). However,
     * the singleton pattern is used here to minimize the number of instances of this class since all of them will
     * behave the same. It is irrelevant if the system ended up with more than one instance of this class.
     */
    ...
    /**
     * Different providers might treat {@code null} predicates differently (EclipseLink and Hibernate do). This is a
     * convenient method to unify such behavior. In this implementation a {@code null} predicate represents a tautology
     * (it is considered {@code true}).
     * 
     * @param builder
     *            criteria builder
     * @param predicates
    ...
    public Predicate and(CriteriaBuilder builder, Predicate... predicates) {
        List<Predicate> nonNullPredicates = new ArrayList<Predicate>(
                predicates.length);
     ...
     * @param builder
     *            criteriaBuider object for creating the predicate
     * @param root
     *            the root element type of the table
     * @return predicate to use in a query
     */
    public <D> Predicate getPredicate(EqualityCondition<D> condition,
            SingularAttribute<? super P, D> attribute, CriteriaBuilder builder,
            Root<P> root) {
        if (condition == null) {
            return getTautology(builder);
        }
        ...
        switch (condition.getMode()) {
            case EQUAL:
                if (condition.getValue() != null) {
                    predicate = builder.equal(root.get(attribute),
                            condition.getValue());
                } else {
            ...
            case UNEQUAL:
                if (condition.getValue() != null) {
                    predicate = builder.notEqual(root.get(attribute),
                            condition.getValue());
                } else {
     ...
     * @return predicate to use in a query
     */
    public <D extends Comparable<D>> Predicate getPredicate(
        ...
        switch (condition.getMode()) {
            case LESS_THAN:
                predicate = builder.lessThan(root.get(attribute),
                        condition.getValue());
                break;
            case LESS_THAN_OR_EQUAL_TO:
                predicate = builder.lessThanOrEqualTo(root.get(attribute),
                        condition.getValue());
                break;
            case EQUAL:
                predicate = builder.equal(root.get(attribute),
            ...
            case GREATER_THAN_OR_EQUAL_TO:
                predicate = builder.greaterThanOrEqualTo(root.get(attribute),
                        condition.getValue());
                break;
            case GREATER_THAN:
                predicate = builder.greaterThan(root.get(attribute),
                        condition.getValue());
                break;
        }
    ...
    public <D extends Comparable<D>> Predicate getPredicate(
        ...
        if (condition.getMode() == IntervalCondition.Mode.NOT_IN
                && interval.getType() == Interval.Type.UNBOUNDED) {
            return getContradiction(builder);
        }
        ...
        switch (interval.getType()) {
            case OPEN:
                leftSideOfInterval = builder.greaterThan(root.get(attribute),
                        interval.getLeftEndpoint());
                rightSideOfInterval = builder.lessThan(root.get(attribute),
                        interval.getRightEndpoint());
                predicate = builder
                        .and(leftSideOfInterval, rightSideOfInterval);
                break;
            case CLOSED:
                leftSideOfInterval = builder.greaterThanOrEqualTo(
                        root.get(attribute), interval.getLeftEndpoint());
                rightSideOfInterval = builder.lessThanOrEqualTo(
                        root.get(attribute), interval.getRightEndpoint());
                predicate = builder
                        .and(leftSideOfInterval, rightSideOfInterval);
                break;
     ...
     * @return predicates to use in a query
     */
    public Predicate getPredicate(TimePeriodCondition condition,
            SingularAttribute<? super P, Date> attribute,
            CriteriaBuilder builder, Root<P> root) {

        IntervalCondition<Date> intervalCondition = null;

        if (condition != null) {
            Interval<Date> interval = Interval.closed(condition.getValue()
                    .getStartTime(), condition.getValue().getEndTime());
     ...
     * @return predicates to use in a query
     */
    public <D> Predicate getPredicate(SetCondition<D> condition,
            SingularAttribute<? super P, D> attribute, CriteriaBuilder builder,
            Root<P> root) {
        if (condition == null) {
            return getTautology(builder);
     ...
     * @return predicates to use in a query
     */
    public Predicate getPredicate(StringCondition condition,
        ...
        switch (condition.getMode()) {
            ...
            case STARTS_WITH:
                ...
                predicate = builder.like(root.get(attribute),
     ...
     * @return predicate to use in a query
     */
    public <T, I extends Serializable> Predicate getPredicateForId(
            EqualityCondition<I> condition,
            SingularAttribute<? super P, I> attribute, CriteriaBuilder builder,
            Root<P> root) {
        if (condition == null) {
            return getTautology(builder);
        }

        EqualityCondition<I> idCondition = condition.convert(IdentityConverter
                .<I> getInstance());
        return getPredicate(idCondition, attribute, builder, root);
    }
     ...
     * @return predicate to use in a query
     */
    public <T, I extends Serializable, R> Predicate getPredicateForRelatedEntity(
            EqualityCondition<I> condition,
            SingularAttribute<? super P, R> attribute, CriteriaBuilder builder,
            Root<P> root) {
        /*
         * TODO: R should be an entity. Before it was SingularAttribute<? super P, ? extends Storable<I>>
         */
        if (condition == null) {
            return getTautology(builder);
        }
