...
import org.opendaylight.persistence.jpa.impl.dao.JpaUtil.PredicateProvider;
...
public abstract class JpaDao<I extends Serializable, T extends Identifiable<I>, P, F, S>
    ...
    protected List<P> findEntities(final F filter,
        ...
        List<Sort<SingularAttribute<? super P, ?>>> convertedSortSpecification = new ArrayList();
        for (Sort<S> sortComponent : sortSpecification) {
            Sort<?> sort = null;
            if (sortComponent.order().equals(SortOrder.ASCENDING)) {
                sort = Sort.ascending(getSingularAttribute(sortComponent.by()));
            } else {
                sort = Sort
                        .descending(getSingularAttribute(sortComponent.by()));
            }
            convertedSortSpecification
                    .add((Sort<SingularAttribute<? super P, ?>>) sort);

        }
        return JpaUtil.find(getEntityClass(), predicateProvider,
                convertedSortSpecification, context);
    }
