 ...
 * layers (DTO). A DTO (Here, {@link Identifiable})
 * should be a type-safe POJO with object value types for attributes when appropriated.
 * The DAO should internally use an object that the underlying database technology understands
 * (Like an Entity in JPA-based implementations for example) where attributes are directly
 * translated to the database native data types. In this way the internals of the
 * storable object are not exposed (For example, changes in column names should
 * not affect the DTO).
 * <p>
...
public interface BaseDao<I extends Serializable, T extends Identifiable<I>, C> {
     ...
     *             underlying database or data store provider is violated, for example:
     *             "Primary Key Integrity Constraint", "Unique Fields", "Non-nullable Fields", etc.
     * @throws PersistenceException if persistence errors occur while executing the operation
     */
    T add(@Nonnull T identifiable, @Nonnull C context) throws IntegrityConstraintViolationException,
     ...
     * @throws IntegrityConstraintViolationException if an integrity constraint defined in the
     *             underlying database or data store provider is violated, for example:
     *             "Unique Fields", "Non-nullable Fields", etc.
     * @throws PersistenceException if persistence errors occur while executing the operation
     */
    T update(@Nonnull T identifiable, @Nonnull C context) throws IntegrityConstraintViolationException,
            PersistenceException;
